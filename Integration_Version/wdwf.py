...
# Global Variables -------------------------------------------------------------------------------------------------------------------
wavelet_name = "db6" # Wavelet to use in the DWTs (6-level or 7-level) or in the 22-bands WPA. Referenced in denoising(),
					 # in wpa_implementation_22_bands() and in from_wavelet_domain_to_time_domain().

wavelet_mode = "per" # Signal extension mode for decomposition and reconstruction in the DWTs (6-level or 7-level)
					 # or in the 22-bands WPA. Referenced in denoising(), in wpa_implementation_22_bands()
					 # and in from_wavelet_domain_to_time_domain().
# ------------------------------------------------------------------------------------------------------------------------------------

def wpa_implementation_22_bands(signal, max_level):
...
def from_wavelet_domain_to_time_domain(signal_wavelet_coefficients, is_dwt, max_level):
...
def denoising(noisy_signal, noise_footprint, sampling_rate, wdwf_id, in_use, is_dwt, dwt_level, a, b, c, d, dps):
...
Inputs:
  noisy_signal: A 1-D numpy array that represents the noisy signal.
...
Outputs:
  ...
  If the selected wavelet analysis is unattainable, denoised_signal.shape[0] = 0.
...
Biomedical Signal Processing and Control, volume 1, issue 3, pages 177-218, July 2006."""
	...
	if is_dwt:
		# Checking if the selected DWT is feasible. ----------------------------------------------------------------------------------
		if (noisy_signal_max_level < dwt_level) or (noise_footprint_max_level < dwt_level):
			denoised_signal = numpy.array([], dtype = encoding_type)
		...
		# Checking if the 22-bands WPA is feasible. ----------------------------------------------------------------------------------
		if (noisy_signal_max_level < dwt_level) or (noise_footprint_max_level < dwt_level):
			denoised_signal = numpy.array([], dtype = encoding_type)
			return denoised_signal
		# ----------------------------------------------------------------------------------------------------------------------------
		...
		noisy_signal_wc = wpa_implementation_22_bands(new_noisy_signal, dwt_level)
		new_noise_footprint = signal_adaptation_for_22_bands_wpa(noise_footprint, dwt_level)
		noise_footprint_wc = wpa_implementation_22_bands(new_noise_footprint, dwt_level)
	...
	denoised_signal = from_wavelet_domain_to_time_domain(denoised_signal_wc, is_dwt, dwt_level)
