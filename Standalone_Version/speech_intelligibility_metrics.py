...
Because the Short Time Fourier Transform (STFT) windows have 32 ms duration (see metric_computation()), the lowest frequency
that can complete a period is 31.25 Hz. In order to account for that, the first band begins from 30 Hz.
...
patterns. The Journal of the Acoustical Society of America, volume 74, issue 3, page 750."""
	...
	roex_filter_lower_bound = 0.000001 # 10 * log10(0.000001) = -60 dB.
	critical_filter_lower_bound = 0.001 # 20 * log10(0.001) = -60 dB.
...
Outputs:
  ...
  Each element represents the value of the SDR at a specific band and time moment."""
	...
	sdr_upper_bound = 15.0 # In dB.
	sdr_lower_bound = -15.0 # In dB.
...
Outputs:
  metric: The value of the metric that was selected, which is between 0.0 and 1.0."""
	...
	if (metric_id != 3) and (metric_id != 4) and (metric_id != 5):
		rms_low_bound_constant_1 = 0.3163 # 20 * log10(0.3163) = -10 dB.
		rms_low_bound_constant_2 = 0.0316 # 20 * log10(0.0316) = -30 dB.
...
def metric_computation(clean_signal_wav_file, denoised_signal_wav_file, metric_id):
...
The clean_signal_wav_file and the denoised_signal_wav_file must have the same sampling rate.
The clean_signal_wav_file and the denoised_signal_wav_file must be coded in the same way.
The clean_signal_wav_file and the denoised_signal_wav_file must be aligned and have the same duration.

The clean_signal_wav_file must not be zero-signal.
The denoised_signal_wav_file could be zero-signal.

Inputs:
  clean_signal_wav_file: A wav file that represents the single-channel clean signal.
  The wav file may be at 16-bit PCM format, but this is not obligatory.

  denoised_signal_wav_file: A wav file that represents the single-channel denoised signal.
  The wav file may be at 16-bit PCM format, but this is not obligatory.
...
Outputs:
  ...
  If metric = None, then an unpleasant situation came to surface.
...
[2] Loizou, P.C. (2013). SPEECH ENHANCEMENT: Theory and Practice, Second Edition (page 560). Boca Raton, FL: CRC Press."""
	...
	clean_signal_sampling_rate, clean_signal = wavfile.read(clean_signal_wav_file)
	denoised_signal_sampling_rate, denoised_signal = wavfile.read(denoised_signal_wav_file)

	# Checking if the clean_signal_wav_file and the denoised_signal_wav_file are single-channel. -------------------------------------
	if (len(clean_signal.shape) != 1) or (len(denoised_signal.shape) != 1):
		print("")
		print("speech_intelligibility_metrics.metric_commputation():")
		print("The " + clean_signal_wav_file + " or/and the " + denoised_signal_wav_file + " are not single-channel!")
		print("")
		return
	# --------------------------------------------------------------------------------------------------------------------------------

	# Checking if the clean_signal_wav_file and the denoised_signal_wav_file have the same sampling rate. ----------------------------
	if clean_signal_sampling_rate != denoised_signal_sampling_rate:
		print("")
		print("speech_intelligibility_metrics.metric_computation():")
		print("The " + clean_signal_wav_file + " and the " + denoised_signal_wav_file + " have not the same sampling rate!")
		print("")
		return

	else:
		sampling_rate = clean_signal_sampling_rate

		if sampling_rate != 16000:
			print("")
			print("speech_intelligibility_metrics.metric_computation():")
			print("The sampling rate of the " + clean_signal_wav_file + " and of the " + denoised_signal_wav_file + " is not 16 kHz.")
	# --------------------------------------------------------------------------------------------------------------------------------

	# Checking if the clean_signal_wav_file and the denoised_signal_wav_file are coded in the same way. ------------------------------
	if clean_signal.dtype != denoised_signal.dtype:
		print("")
		print("speech_intelligibility_metrics.metric_computation():")
		print("The " + clean_signal_wav_file + " and the " + denoised_signal_wav_file + " are not coded in the same way!")
		print("")
		return

	else:
		encoding_type = clean_signal.dtype

		if encoding_type != numpy.int16:
			print("")
			print("speech_intelligibility_metrics.metric_computation():")
			print("The " + clean_signal_wav_file + " and the " + denoised_signal_wav_file + " are not at 16-bit PCM format.")
	# --------------------------------------------------------------------------------------------------------------------------------

	# Checking if the clean_signal_wav_file and the denoised_signal_wav_file have the same duration. ---------------------------------
	...
	# Checking if the clean_signal_wav_file is zero-signal. --------------------------------------------------------------------------
	clean_signal_list = list(clean_signal)

	if clean_signal_list.count(0.0) == len(clean_signal_list):
		print("")
		print("speech_intelligibility_metrics.metric_computation():")
		print("The " + clean_signal_wav_file + " is zero-signal!")
		print("")
		return
	# --------------------------------------------------------------------------------------------------------------------------------

	window_type = "hamming"
	window_duration = 0.032 # In seconds. If sampling_rate = 16 kHz, 0.032 sec <=> 512 samples = 2^9 samples.
	...
	if time_moments.shape[0] <= 2:
		...
		print("")
		print("speech_intelligibility_metrics.metric_computation():")
		print("The number of windows at the STFT computation of the " + clean_signal_wav_file + " is not enough!")
		print("")
		return
